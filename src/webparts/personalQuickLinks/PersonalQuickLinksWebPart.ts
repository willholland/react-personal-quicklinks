...
import styles from './components/PersonalQuickLinks.module.scss';
...
export default class PersonalQuickLinksWebPart extends BaseClientSideWebPart<IPersonalQuickLinksWebPartProps> {
  ...
  private _itemPaneAdding: boolean = true; // 0 = Adding new item. 1 = Editing existing item.
  ...
  public render(): void {
    const element: React.ReactElement<IPersonalQuickLinksProps> = React.createElement(
      PersonalQuickLinks,
      {
        ...
        handlePropertyChange: this.onPropertyPaneFieldChanged.bind(this),
  ...
  private addLink(link:IQuickLink): void{
    let index = this.properties.links.push(link) - 1;
    this.editLink(index);
  }

  private editLink(index:number): void{
  ...
  private getItemPropertyPaneConfiguration(): IPropertyPaneConfiguration {
    ...
    let QuickLink_title = PropertyPaneTextField(`links[${this.activeIndex}].title`, {
      label: strings.Title,
    });

    let QuickLink_url = PropertyFieldFilePicker(`links[${this.activeIndex}].url`, {
      key: "filePickerID",
      context: this.context,
      ...
      label: strings.Link,
      buttonLabel: "Change",
      buttonClassName: styles.filePickerChangeButton,
      buttonIcon: ""
    });

    let QuickLink_thumbNailUrl = PropertyPaneTextField("thumbNailUrl", {
        label: strings.Thumbnail,
    });

    let QuickLink_alternateText = PropertyPaneTextField("alternateText", {
        label: strings.AlternateText,
    });

    let QuickLink_description = PropertyPaneTextField("description", {
    ...
    const props = [
      ...
      [ 
        QuickLink_url
        , QuickLink_title
        , QuickLink_thumbNailUrl
        , QuickLink_alternateText
      ], 
      // Filmstrip Layout Properties
      [],
      // Grid Layout Properties
      ...
      pages:[
        {
          ...
          groups: [
            {
              groupName: strings.Link,
              groupFields: props[this.properties.layout] as []
            }
          ]
        }
      ]
